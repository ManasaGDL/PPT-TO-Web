...
if(week?.length==0 || (week)=='undefined')
{
    ...
    data.map(row=>{
     Object.keys(row).map(key=>{
      ...
      if(key.trim().toLowerCase()==="week")
      {
        ...
        { setMonth(Object.values(row)[0])
          setWeekData(Object.values(row).slice(2))
          setMetrics(Object.keys(row).slice(2))
        }
      }
     })
    })
              ...
              <Grid container alignItems="center" justifyContent="space-between">
                <Grid item>
                  <Grid container direction="column" spacing={1}>
                    <Grid item>
                    <Typography variant="subtitle1" color={"red"}>{month}</Typography>
                      <Typography variant="subtitle1">Week - <span style={{ color:"#0B60B0"}}>{week}</span></Typography>
                    </Grid>
                    <Grid item>
                      <Typography variant="h3">Total Tasks worked  = {totalTasks} </Typography>
                    </Grid>
                  </Grid>
                </Grid>
                <Grid item>
                 <Grid item>
                  ...
                  <Grid item>
                  <TextField id="standard-select-currency"  select value={week} onChange={(e) => 
